...
class RCControl(object):
    ...
    def steer(self, prediction):
        if prediction == 2:
            self.serial_port.write(str(1))
            print("Forward")
        elif prediction == 0:
            self.serial_port.write(str(4))
            print("Left")
        elif prediction == 1:
            self.serial_port.write(str(3))
            print("Right")
        else:
            self.stop()

    def stop(self):
        self.serial_port.write(str(0))
...
class ThreadServer(object):
    ...
    distance_thread = threading.Thread(target=server_thread2, args=('192.168.1.102', 22))
    distance_thread.start()
    video_thread = threading.Thread(target=server_thread('192.168.1.102', 20))
    video_thread.start()
